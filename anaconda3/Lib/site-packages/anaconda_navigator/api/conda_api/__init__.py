...
def normalize_default_channels(rc: dict[str, typing.Any]) -> dict[str, typing.Any]:
    """Convert Null/Empty `default_channels` to default empty list."""
    rc['default_channels'] = rc.get('default_channels') or []
    return rc
...
class _CondaAPI(QObject):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def install(
        ...
        if force:
            cmd_list.append('--force-reinstall')
    ...
    def generate_config_path(
        ...
        :param path: Path to conda configuration file.
        :param prefix: Prefix path, to retrieve the specific prefix configuration file.
        :param system: Retrieve the system configuration file.
        """
        if path:
            return path
    ...
    def load_rc(
        ...
        Depending on path or specific environment prefix and system that config file will be returned. If invalid or
        nonexistent file, then an empty dictionary is returned.

        :param path: Path to conda configuration file.
        :param prefix: Prefix path, to retrieve the specific prefix configuration file.
        :param system: Retrieve the system configuration file.
        """
        config_path: str | None = self.generate_config_path(path, prefix, system)

        data: dict[str, typing.Any] = {}
        if config_path and os.path.isfile(config_path):
            with open(config_path, 'rt', encoding='utf8') as stream:
                data = yaml.full_load(stream) or {}

        data = normalize_default_channels(data)
    ...
    def get_platform():
        ...
        if arch_name in non_x86_machines:
            return f'{sys_platform}-{arch_name}'

        return f'{sys_platform}-{bits}'
