...
class _AnacondaAPI(QObject):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def conda_data(self, prefix=None):
        ...
        def _conda_info_processed(worker, info, error):
            ...
            if repodata:
                ...
                new_worker = self._client_api.load_repodata(repodata=repodata, metadata=self._metadata)
                new_worker.base_worker = base_worker
    ...
    def process_apps(  # pylint: disable=too-many-locals
        ...
        for app_name, app_data in itertools.chain(apps.items(), missing_apps.items()):
            ...
            application_patch = collected_applications.app_patches.get(app_name, external_apps.EMPTY_PATCH)
            ...
            description: str = (
                app_data.get('description', None) or
                linked_app_info.get('description', None) or
                local_app_info.get('description', None) or
                ''
            )
            image_path: str = (
                    linked_app_info.get('image_path', None) or
                    local_app_info.get('image_path', None) or
    ...
    def install_packages(  # pylint: disable=too-many-arguments
            self, prefix, pkgs, dry_run=False, no_default_python=False, force=True):
